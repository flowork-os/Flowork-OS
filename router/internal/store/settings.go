...
type BrainConfig struct {
	...
	MaxSkillBodyChars int    `json:"maxSkillBodyChars"` // per-skill body truncation; 0 = no cap (legacy)
...
func PatchSettings(d *sql.DB, patch map[string]any) (*Settings, error) {
	...
	curMap := map[string]any{} // non-nil: curJSON corrupt → tanpa init, write loop bawah PANIC
