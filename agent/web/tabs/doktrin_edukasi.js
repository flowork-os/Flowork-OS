...
// 2026-06-06 (owner-approved "hajar"): PER-AGENT — render(mainEl, agentId) reads
//   /api/agents/edu-errors?id=<agent> (agent's OWN store, §C) instead of the global
//   shim; moved from a global sidebar tab to a per-agent button. Migrasi mr-flow →
...
export async function render(mainEl, agentId = 'mr-flow') {
  loadStyle('doktrin_edukasi', CSS);

  mainEl.innerHTML = `
    <h2>${esc(L.title)} — ${esc(agentId)}</h2>
  ...
  let entries = [];
  try {
    // Per-agent error-edu from the agent's OWN store (§C), not the global shim.
    // Map the raw row shape → the display shape this tab already renders.
    const resp = await fetchJSON('/api/agents/edu-errors?id=' + encodeURIComponent(agentId));
    ...
    entries = items.map(it => ({
      ...
      title: it.title || '',
      error_code: it.code,
      message_template: it.explanation || '',
      evolution_hint: it.remediation || '',
    }));
  } catch (e) {
  ...
  function openEditModal(entry) {
    ...
    modal.querySelector('#deSave').addEventListener('click', async () => {
      ...
      try {
        const r = await fetch('/api/agents/edu-errors?id=' + encodeURIComponent(agentId), {
          method: 'POST',
