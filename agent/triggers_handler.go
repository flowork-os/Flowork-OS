// === LOCKED FILE (soft) — STABLE. Audit rilis 2026-06-14 (menu Schedule+Trigger, §36 TUTORIAL): aman. ===
// auth-gated; webhook secret constant-time + tolak-kosong + dedup; id anti-traversal; + fitur Duplicate. DO NOT MODIFY tanpa owner.
...
// triggersDuplicateHandler — POST /api/triggers/duplicate?id=<src>. Clone a schedule/
// trigger into a fresh, UNIQUE id (src-copy, src-copy2, …), name "(copy)", DISABLED by
// default (so the clone never double-fires until the owner turns it on), and — for
// webhook types — a NEW secret (never reuse the source's). Owner request 2026-06-14.
func triggersDuplicateHandler(eng *triggers.Engine) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			tfWriteJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "POST only"})
			return
		}
		src := strings.TrimSpace(r.URL.Query().Get("id"))
		if !triggerIDRe.MatchString(src) {
			tfWriteJSON(w, http.StatusBadRequest, map[string]any{"error": "id invalid"})
			return
		}
		orig, err := eng.Store.GetTrigger(src)
		if err != nil || orig == nil {
			tfWriteJSON(w, http.StatusNotFound, map[string]any{"error": "trigger tak ditemukan: " + src})
			return
		}
		// id baru yang UNIK + tetap slug valid (≤40 char). Sisakan ruang utk "-copyNN".
		base := src
		if len(base) > 32 {
			base = base[:32]
		}
		...
		for i := 1; i <= 99; i++ {
			cand := base + "-copy"
			if i > 1 {
				cand = base + "-copy" + strconv.Itoa(i)
			}
			if !triggerIDRe.MatchString(cand) {
				continue
			}
			if cur, _ := eng.Store.GetTrigger(cand); cur == nil {
				newID = cand
				break
			}
		}
		if newID == "" {
			tfWriteJSON(w, http.StatusConflict, map[string]any{"error": "tak ada id-copy bebas (terlalu banyak salinan)"})
			return
		}
		secret := ""
		if typ, ok := triggers.GetType(orig.TypeID); ok && typ.Mode() == "webhook" {
			secret = randSecret() // JANGAN reuse secret sumber
		}
		dup := *orig
		dup.ID = newID
		dup.Name = strings.TrimSpace(orig.Name) + " (copy)"
		dup.Enabled = false // clone mulai OFF — anti dobel-fire sampai owner menyalakannya
		dup.WebhookSecret = secret
		dup.State = "" // jangan warisi state poll/dedup sumber
		dup.LastFired, dup.LastStatus, dup.CreatedAt = "", "", ""
		if err := eng.Store.UpsertTrigger(dup); err != nil {
			tfWriteJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
			return
		}
		tfWriteJSON(w, 0, map[string]any{"ok": true, "id": newID, "from": src})
	}
}
