...
func installPluginPack(host *kernelhost.Host, store *floworkdb.Store, raw []byte, approveCaps bool) pluginInstallResult {
	...
	// GERBANG SERAGAM (plug-and-play): dispatch by KIND. Pack tool/slash/scanner
	// punya payload sendiri (wasm / yaml check) → handler masing-masing. Pack TANPA
	// kind (agent/category) jatuh ke jalur agent di bawah (TIDAK berubah).
	var kindPeek struct {
	...
	case "slash":
		b, s := installSlashPack(host, raw)
		return pluginInstallResult{s, b}
	case "scanner":
		b, s := scanapi.InstallScannerPack(raw)
		return pluginInstallResult{s, b}
	case "channel":
