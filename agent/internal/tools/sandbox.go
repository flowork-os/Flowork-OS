...
func SandboxRun(ctx context.Context, t Tool, args map[string]any, opts SandboxOpts) (Result, error) {
	...
	if !opts.SkipCapGate {
		if check := FromCapsChecker(ctx); check != nil {
			cap := t.Capability()
			if cap != "" && !check(cap) {
				return Result{}, fmt.Errorf("%w: tool %q butuh izin %q yang belum kamu punya — ini aturan, bukan salahmu. Petunjuk: jujur bilang ke user kamu belum punya izin ini (JANGAN ngarang hasil); kalau memang perlu, user bisa berikan capability lewat owner/manifest", ErrSandboxCapDenied, name, cap)
			}
		}
	}
