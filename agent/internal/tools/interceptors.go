...
// Audit rilis 2026-06-14 (§38 AI Agent): confinement aman; pesan blok dibuat EDUKATIF (peluk+petunjuk
// +"ini aturan") per hukum owner #4 — jangan marahin agent. Lihat interceptors_edu_test.go.
...
func (workspacePathInterceptor) Before(_ context.Context, _ Tool, args map[string]any) error {
	for k, v := range args {
		...
		if strings.Contains(s, "..") {
			...
			for _, p := range parts {
				if p == ".." {
					return fmt.Errorf("[PETUNJUK, bukan salahmu] path %q pakai '..' (keluar workspace) — diblok demi keamanan, ini aturan tetap. Coba path relatif DI DALAM workspace-mu (mis. \"document/catatan.txt\"), tanpa '..'", k)
				}
			}
		}
		...
		for _, d := range dangerous {
			if strings.Contains(lower, strings.ToLower(d)) {
				return fmt.Errorf("[PETUNJUK, bukan salahmu] path %q menyentuh lokasi sistem terlindungi %q — diblok demi keamanan, ini aturan tetap. Kerjakan file di workspace-mu saja (kategori: tools/job/document/media/cache/log)", k, d)
			}
		}
	}
	return nil
...
func (sensitiveFileInterceptor) Before(_ context.Context, _ Tool, args map[string]any) error {
	check := func(s string) error {
		...
		if sensitiveBasenames[bl] {
			return fmt.Errorf("[PETUNJUK, bukan salahmu] file %q itu rahasia (kredensial/kunci) jadi diblok demi keamanan — ini aturan tetap. Kalau butuh nilai rahasia, minta lewat tool secret/owner, jangan baca filenya langsung", base)
		}
		for _, suf := range sensitiveSuffixes {
			if strings.HasSuffix(bl, suf) {
				return fmt.Errorf("[PETUNJUK, bukan salahmu] file berakhiran %q biasanya kunci/sertifikat rahasia jadi diblok — ini aturan tetap. Pakai jalur secret resmi kalau perlu nilainya", suf)
			}
		}
		return nil
	}
	...
	for k, v := range args {
		...
		for _, p := range personaInjectionPatterns {
			if strings.Contains(lower, p) {
				return fmt.Errorf("[PETUNJUK, bukan salahmu] arg %q memuat pola yang mirip suntikan-prompt (%q) jadi diblok demi keamanan — ini aturan tetap. Kalau itu cuma teks data biasa, tulis ulang tanpa frasa perintah seperti itu", k, p)
			}
		}
	}
	return nil
