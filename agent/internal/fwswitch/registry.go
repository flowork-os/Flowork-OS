...
var Registry = []Switch{
	...
	{"FLOWORK_MESH_VERIFY_PROMOTE", "Verify-on-promote mesh", "ON (default) = sebelum knowledge mesh di-promote (auto-pipeline), integritas NODE LOKAL (tier-2 super_scrit) di-cek dulu — node ke-tamper → promote DIBLOK (jangan nyerep ilmu ke otak yg mungkin dibajak). Endpoint approve-verified juga re-verify tanda-tangan paket asli. OFF = balik perilaku lama (promote tanpa cek integritas). Bagian FASE M lockbox.", "bool", "true", "Mesh / Lockbox"},
	{"FLOWORK_MESH_EVOLUSI_ALLOW", "Izinin mesh → evolusi (BAHAYA)", "OFF (default) = TEMBOK NAIK: knowledge dari peer mesh (source=mesh_federated) HARAM nembus ke loop evolusi/codegen self-improve (biar racun federasi ga nyampe ke kode yg di-generate). ON = longgarin (sadar-risiko, owner-only) → mesh knowledge boleh masuk konteks evolusi. Default aman = OFF. Bagian FASE M lockbox.", "bool", "false", "Mesh / Lockbox"},
	{"FLOWORK_WAKE_MANDOR", "Wake-push mandor (by-event)", "ON (default) = tiap worker SELESAI dikerjain via agent_command, mandor langsung dibangunin (RunNow rule worklog-pending) tanpa nunggu poll — reaksi instan. Debounce 60 detik (banyak worker kelar beruntun = 1 wake), anti self-loop (rule yang target-nya worker itu di-skip). Poll trigger tetap jalan sbg jaring pengaman. OFF = poll-only (perilaku lama). Ga ada rule worklog-pending aktif → no-op.", "bool", "true", "Router / Tools"},
	{"FLOWORK_TIER3_TAIL", "Tier-3 volatile ke ekor (cache bp-4)", "ON (default) = blok volatile Tier-3 mr-flow (waktu/memory/recall/tugas-aktif) dikirim sbg pesan user TERAKHIR, bukan system ke-2 → system 100% stabil + history bersih → breakpoint ke-4 (jalur Claude) bikin persona+history KE-BACA dari cache lintas-turn (terukur: turn-2 read=5876 create=866). OFF = perilaku lama (volatile di system; history ga pernah read-hit lintas-turn). ⚠️ Nilai nyampe agent pas load → abis ganti, restart stack.", "bool", "true", "Router / Tools"},
	...
	{"FLOWORK_APPROVAL_AUTO", "Auto-approve (tanpa gerbang manual)", "ON (default) = SEMUA antrian approval pending di-approve OTOMATIS (poller cepat) → agent ga ke-block, ga ada notif numpuk. Perlu walau mode bypass: gate sensitive-args (state.db) + sensitive-tool di sandbox (frozen) tetap bikin pending. OFF = balik manual approve di GUI tab Protector.", "bool", "true", "Security / Approval"},
	{"FLOWORK_APPROVAL_NOTIFY", "Notif Telegram approval pending", "ON (default) = tiap ada antrian approval PENDING baru, owner dikabarin via Telegram (batch per menit, dedup — ga spam). OFF = cek manual di GUI tab Protector.", "bool", "true", "Security / Approval"},
	{"FLOWORK_ANTIGRAVITY_CLIENT_ID", "OAuth client_id Antigravity", "client_id OAuth Google buat login Antigravity. Default = client_id resmi Antigravity. Binary app punya BANYAK client_id (gemini-cli/SDK) yg ga bisa dibedain otomatis — jadi di-anchor di sini (editable, GUI=kebenaran). Kosongin = coba extract dari binary (best-effort). Secret di-extract otomatis dari binary app.", "string", "1071006060591-tmhssin2h21lcre235vtolojh4g403ep.apps.googleusercontent.com", "Router / Antigravity"},
	{"FLOWORK_ANTIGRAVITY_MODELS", "Model Antigravity (Gemini via cloud-code)", "Daftar model yg di-advertise provider Antigravity auto (pisah koma). Dipakai pas jalur OAuth (MITM mentok karena app pake DoH). Kosong = provider ga match sampai ada capture MITM. Edit di sini = GUI kebenaran, bukan hardcode. Model asli dari spec Antigravity.", "string", "gemini-3.1-pro-low,gemini-3.5-flash-low", "Router / Antigravity"},
	{"FLOWORK_MODEL_REMAP", "Auto-remap model pensiun", "ON (default) = model yang id-nya UDAH pensiun (mis. claude-2.1, claude-3-opus, gpt-3.5-turbo) otomatis dipetakan ke pengganti hidup + WARNING di log router — bukan diteruskan mentah (yang bikin upstream 404 → gagal). Model hidup TIDAK disentuh. OFF = teruskan apa adanya (buat sengaja test provider lama).", "bool", "true", "Router / Resilience"},
	{"FLOWORK_PROC_GRACE_MS", "Grace shutdown proses anak (ms)", "Lama nunggu tiap TAHAP pas matiin proses anak (app/MCP/adapter): SIGINT→(tunggu)→SIGTERM→(tunggu)→SIGKILL. Default 2500ms/tahap (min 100, cap 30000). Gede = kasih proses lebih lama flush/tutup rapi; kecil = matiin lebih cepat. Ganti kill-keras (SIGKILL langsung) yang bikin data corrupt / port nyangkut.", "int", "2500", "Autonomy / Orchestration"},
