...
type Category struct {
	...
	// WorkerDirective: OPSIONAL. Instruksi cara worker/analis kerja. Kosong =
	// default analysis-shaped ("cari data REAL, jangan ngarang") — jaga
	// backward-compat saham/crypto. Diisi caller buat kategori KREATIF (mis.
	// zodiak: ga ada "data real", worker ngarang ramalan = MEMANG tugasnya).
...
func RunCategoryTask(ctx context.Context, host Invoker, sharedDir string, cat Category, input, runID string, rec Recorder) Result {
	...
	for idx, m := range cat.Crew {
		res.Steps = append(res.Steps, invokeWorker(ctx, host, sharedDir, synthJobDir, taskName, input, runID, m, idx, "", cat.WorkerDirective, rec))
	}
	...
	for round := 0; round < maxRetaskRounds && serr == nil; round++ {
		...
		res.Steps = append(res.Steps, invokeWorker(ctx, host, sharedDir, synthJobDir, taskName, input, runID, *target, synthIdx+1+round, instruction, cat.WorkerDirective, rec))
