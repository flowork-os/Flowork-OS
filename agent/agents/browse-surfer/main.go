...
// time-bound/auto-continue + router-retry ADA di package SHARED `flowork-gui/internal/
// agentkit` (keystone roadmap AGENTKIT). Identitas agent (id, persona, model, tool) =
// RUNTIME (FLOWORK_AGENT_ID + FLOWORK_AGENT_CONFIG dari GUI + self-prompt render host) —
// JADI file ini SAMA buat semua worker; yang beda cuma manifest.json + config DB.
//
